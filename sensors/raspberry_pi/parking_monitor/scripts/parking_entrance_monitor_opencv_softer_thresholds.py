"""
Parking Lot Entrance Monitor (Classical OpenCV Version, Headless, Debug)
...
- Debug mode: writes side-by-side view of frame and fgmask
...
# ===== Tunable parameters =====
DOWNSCALE_WIDTH = 640          # target width for processing (keeps aspect ratio)
MIN_CONTOUR_AREA = 400         # minimum blob area to be considered a vehicle (in downscaled pixels)
...
# Debug: show fgmask next to frame in the output video
...
class ParkingEntranceMonitorCV:
    ...
    def _auto_line_position(self, height_ds):
        ...
        base = int(height_ds * 0.65)  # a bit lower so cars are larger when crossing
    ...
    def process_video(self, save_output=False):
        ...
        if save_output:
            ...
            output_path = "parking_entrance_output_opencv_classical_debug.mp4"
        ...
        while True:
            ...
            # Softer threshold than before to keep more motion
            ...
            # Build debug composite frame (frame + mask side by side) if enabled
            if DEBUG_SHOW_MASK:
                fg_bgr = cv2.cvtColor(fgmask, cv2.COLOR_GRAY2BGR)
                # Resize mask to match frame_small if needed
                if fg_bgr.shape[:2] != frame_small.shape[:2]:
                    fg_bgr = cv2.resize(fg_bgr, (ds_width, ds_height), interpolation=cv2.INTER_NEAREST)
                frame_to_write = np.hstack((frame_small, fg_bgr))
            else:
                frame_to_write = frame_small
        ...
        print("\n" + "=" * 50)
        print("FINAL STATISTICS (Classical OpenCV, Debug)")
        print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(
        description="Parking lot entrance monitor using classical OpenCV (no YOLO, headless, debug)."
    )
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save annotated video to parking_entrance_output_opencv_classical_debug.mp4"
    )
