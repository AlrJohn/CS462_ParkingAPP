"""
Parking Lot Entrance Monitor (Classical OpenCV + ROI on Full Frame, Headless)
...
class ParkingEntranceMonitorCVROI:
    ...
    def _auto_line_position(self, roi_height_ds):
        ...
        # Previously 0.65; now lower line by ~10% -> 0.75
    ...
    def process_video(self, save_output=False):
        ...
        if save_output:
            ...
            output_path = "parking_entrance_output_opencv_roi_fullframe.mp4"
        ...
        while True:
            ...
            # Line goes from left to right of ROI at self.line_position in downscaled ROI
        ...
        print("\n" + "=" * 50)
        print("FINAL STATISTICS (Classical OpenCV + ROI on Full Frame)")
        print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(
        description="Parking lot entrance monitor using classical OpenCV on an ROI (no YOLO, headless, full-frame output)."
    )
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save annotated video to parking_entrance_output_opencv_roi_fullframe.mp4"
    )
