"""
Parking Lot Entrance Monitor (Headless, Extra Optimized)
- Uses smaller YOLO model (yolo11n.pt by default)
- Downscales frames before processing
- Processes every 10th frame (skips 9 in between)
- Tracks processing time and effective FPS
- Safe to run on Ubuntu Server / Raspberry Pi with no display

Output video: parking_entrance_output_downscale_skip9frames_smallmodel.mp4
"""
...
# Process every Nth frame (N=10 means process 1, skip 9)
...
class ParkingEntranceMonitor:
    def __init__(self, video_path, model_path='yolo11n.pt', line_position=None):
        """
        Initialize the parking entrance monitor

        Args:
            video_path: Path to the video file
            model_path: Path to YOLO model (default: yolo11n.pt, smaller & faster)
    ...
    def process_video(self, show_video=False, save_output=False):
        ...
        print(f"Processing every {PROCESS_EVERY_N_FRAMES}th frame "
              f"(skipping {PROCESS_EVERY_N_FRAMES - 1} in between)")
        ...
        if save_output:
            ...
            output_path = 'parking_entrance_output_downscale_skip9frames_smallmodel.mp4'
...
def main():
    parser = argparse.ArgumentParser(
        description='Monitor parking lot entrance for entering/exiting vehicles '
                    '(headless, downscaled, skip 9 frames, small YOLO model)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='yolo11n.pt',
        help='Path to YOLO model (default: yolo11n.pt, smaller & faster)'
    )
    parser.add_argument(
        '--line',
        type=int,
    ...
    parser.add_argument(
        '--save',
        action='store_true',
        help='Save processed video to '
             'parking_entrance_output_downscale_skip9frames_smallmodel.mp4'
    )
