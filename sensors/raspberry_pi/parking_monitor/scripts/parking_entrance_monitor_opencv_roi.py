"""
Parking Lot Entrance Monitor (Classical OpenCV + ROI, Headless, Debug)

- Uses background subtraction + contour detection (no YOLO, no deep learning)
- Crops a region-of-interest (ROI) around the driveway to "zoom in"
...
- Downscales ROI for speed
- Debug mode: writes side-by-side view of ROI frame and fgmask
...
LINE_VERTICAL_OFFSET_PIXELS = -25

# Debug: show fgmask next to ROI frame in the output video
...
class ParkingEntranceMonitorCVROI:
    def __init__(self, video_path, line_position=None):
        ...
        # user spec: left ~1/3, right ~3/4, top ~1/3, bottom ~3/4
        self.x_left_frac = 1.0 / 3.0
        self.x_right_frac = 3.0 / 4.0
        self.y_top_frac = 1.0 / 3.0
        self.y_bottom_frac = 3.0 / 4.0
    ...
    def _auto_line_position(self, roi_height_ds):
        ...
        base = int(roi_height_ds * 0.65)  # lower so cars are larger when crossing
    ...
    def process_video(self, save_output=False):
        ...
        roi_width = self.roi_x_right - self.roi_x_left
        roi_height = self.roi_y_bottom - self.roi_y_top
        print(f"ROI (original coords): x[{self.roi_x_left}:{self.roi_x_right}], "
              f"y[{self.roi_y_top}:{self.roi_y_bottom}] => {roi_width}x{roi_height}")

        # Compute downscaled ROI dimensions
        roi_scale = DOWNSCALE_WIDTH_ROI / float(roi_width)
        ds_width = DOWNSCALE_WIDTH_ROI
        ds_height = int(roi_height * roi_scale)
        ...
        # Decide output frame size (with or without debug mask)
        if DEBUG_SHOW_MASK:
            out_width = ds_width * 2
            out_height = ds_height
        else:
            out_width = ds_width
            out_height = ds_height

        # Video writer (downscaled ROI / debug)
        out = None
        if save_output:
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            output_path = "parking_entrance_output_opencv_roi_debug.mp4"
        ...
        while True:
            ...
            # Softer threshold to keep more motion
            ...
            # Draw entrance line within ROI (downscaled)
            cv2.line(
                roi_small,
            ...
            for track in self.tracks.values():
                ...
                # Draw centroid
                cv2.circle(roi_small, (int(cx), int(cy)), 4, (0, 255, 255), -1)
                # Draw ID label
                cv2.putText(
                    roi_small,
                    f"ID:{track.id}",
                ...
                if len(track.history) > 1:
                    ...
                    cv2.polylines(roi_small, [pts], False, (230, 230, 230), 2)

            # Draw detections (bounding boxes) in ROI
            for x, y, w, h, cx, cy in detections:
                cv2.rectangle(roi_small, (x, y), (x + w, y + h), (255, 0, 0), 2)
            ...
            # Build debug composite frame (ROI + mask side by side) if enabled
            if DEBUG_SHOW_MASK:
                fg_bgr = cv2.cvtColor(fgmask, cv2.COLOR_GRAY2BGR)
                # Ensure same size
                if fg_bgr.shape[:2] != roi_small.shape[:2]:
                    fg_bgr = cv2.resize(fg_bgr, (ds_width, ds_height), interpolation=cv2.INTER_NEAREST)
                frame_to_write = np.hstack((roi_small, fg_bgr))
            else:
                frame_to_write = roi_small
        ...
        print("\n" + "=" * 50)
        print("FINAL STATISTICS (Classical OpenCV + ROI, Debug)")
        print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(
        description="Parking lot entrance monitor using classical OpenCV on an ROI (no YOLO, headless, debug)."
    )
    ...
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save annotated video to parking_entrance_output_opencv_roi_debug.mp4"
    )
